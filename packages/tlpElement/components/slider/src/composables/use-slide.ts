...
export const useSlide = (
    ...
    const barSize = computed(() => {
        return props.range
            ...
            : `${
                  (100 * (initData.firstValue - props.min)) /
                  (props.max - props.min)
              }%`;
    });
    ...
    const getButtonRefByPercent = (
        ...
        const targetValue =
            props.min + (percent * (props.max - props.min)) / 100;
        if (!props.range) {
            return firstButton;
        }
    ...
    const handleSliderPointerEvent = (
        ...
        if (props.vertical) {
            ...
            const sliderOffsetBottom =
                slider.value!.getBoundingClientRect().bottom;
            newPercent =
                ((sliderOffsetBottom - clientY) / initData.sliderSize) * 100;
        } else {
            const clientX =
            ...
            newPercent =
                ((clientX - sliderOffsetLeft) / initData.sliderSize) * 100;
        }
